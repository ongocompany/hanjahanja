...
- ✅ 확장 프로그램 내 Supabase 로그인 연동 (이메일/비밀번호) 🤖
- ✅ OAuth 소셜 로그인 (Google, 카카오) — 웹 + 확장 🤝 (네이버는 사업자 필요)
...
- ✅ 합성 데이터 생성 (소수 의미 불균형 해소) — **2,237개 단어 대상**
  ...
  - ✅ Gemini 2.5-flash-lite: 1,999단어 / 400배치 118,874샘플 완료
...
- ✅ 합성 데이터 병합 후 **재학습** 완료 (model-v1, klue/roberta-base, test_acc 96.57%)
...
- ✅ **ONNX v4 변환** (합성 데이터 재학습 model-v1) — INT8 112MB, 헤드 33MB, API 서버 교체 완료
...
- ✅ 레벨 판정 알고리즘 🤖
- ✅ 결과 페이지 (SNS 공유 버튼 미구현) 🤖

### Day 20-21: 마이페이지 + 단어장
- ✅ 레벨 설정 변경 🤖
- ✅ 계정 설정 (닉네임, 로그아웃) 🤖
- ✅ 마이페이지 단어장 페이지 (/mypage) 🤖
  - ✅ 단어장 목록 (날짜별 그룹핑, 검색, 필터) 🤖
  - ✅ 단어장 삭제 기능 🤖
  - ✅ "외웠어요" 체크 기능 🤖
  - ✅ 단어장 연습문제 (확장 팝업 퀴즈: 노출/클릭 기반 + 뜻풀이 힌트 + 오답 재출제) 🤖
- ⬜ 학습 통계 페이지 (노출/클릭 데이터 시각화) 🤖
...
### Day 24-25: 배포
- ✅ Vultr VPS 배포 (pm2 + nginx, Docker 대신) 🤝
- ✅ hanjahanja.co.kr SSL 인증서 설정 (Let's Encrypt) 🤝
- ✅ WSD API 서버 VPS 배포 (FastAPI + ONNX Runtime) 🤝
- ✅ 크롬 웹 스토어 등록 신청 (심사 대기 중) 🧑

### 어드민 페이지
- ⬜ 어드민 대시보드 기본 구조 (접근 제어, 레이아웃) 🤖
- ⬜ 오변환 신고 관리 (목록, 승인/거부, 학습 데이터 export) 🤖
- ⬜ 사용자 통계 (가입자 수, 활성 사용자, 노출/클릭 집계) 🤖
- ⬜ WSD 모델 성능 모니터링 (단어별 정확도, 오변환 빈도) 🤖
...
| M3.5 | WSD 모델 | 동음이의어 문맥 판별 모델 학습 + ONNX 변환 + 확장 통합 | ✅ (합성데이터 재학습 96.57%, ONNX v4 변환 + API 서버 교체 완료) |
| M4 | 진단 테스트 완성 | 테스트→레벨 판정→결과 | ✅ (문제 데이터 + UI + 레벨 판정 + 결과 페이지) |
| M5 | MVP 통합 완성 | 웹+확장 전체 플로우 | 🔄 (오변환 신고 추가, 어드민 페이지 남음) |
| M6 | 배포 완료 | hanjahanja.co.kr 라이브 | ✅ (VPS + 크롬 웹스토어 심사 중) |
